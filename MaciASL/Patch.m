...
#else //PATCHMATIC
-(void)patchText:(NSMutableString *)text apply:(bool)apply {
    [self prepareWithText:text];
    if (apply) {
        for (NSArray *changes in _changes)
            for (PatchDelta *change in changes)
                [text replaceCharactersInRange:change.before withString:change.after];
    }
}
#endif //PATCHMATIC

#if PATCHMATIC
-(PatchFileStats*)patchStats {
    PatchFileStats* stats = [PatchFileStats alloc];
    stats.patches = (int)[_patches count];
    int count = 0;
    for (NSArray *changes in _changes)
        count += [changes count];
    stats.changes = count;
    stats.rejects = (int)_rejects;
    return stats;
}
#endif
...
#else
                    [list addObject:@{@"before":[[NSString alloc] initWithString:[_text substringWithRange:before]], @"after":[[NSString alloc] initWithString:delta.after]}];
#endif
                    _rejects++;
...
#else
    NSMutableString *before = [[NSMutableString alloc] initWithString:[_text substringWithRange:context]];
